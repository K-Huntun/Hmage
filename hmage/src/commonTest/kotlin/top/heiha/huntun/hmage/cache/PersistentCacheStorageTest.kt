...
class PersistentCacheStorageTest {
    ...
    private val cachePath = "/data/hmage/cache".toPath()
    ...
    @Test
    fun testCreateDirs() {
        fakeFileSystem.createDirectories(cachePath)
    }
    ...
    fun testPut() {
        ...
        runBlocking {
            ...
            assertTrue(
                fakeFileSystem.exists(storage.generateCachePath(url)),
        ...
        try {
            runBlocking {
                ...
                assertNull(
                    ...
                    "The cache with url1 as key should be removed"
                )
                assertNotNull(
                    storage.find(url2, emptyMap()),
                    "The cache with url2 as key should be retain"
                )
            }
        } finally {
            runBlocking {
    ...
    fun testPriority() {
        try {
            ...
            runBlocking {
                ...
                assertNotNull(
                    storage.find(url1, emptyMap()),
                    "The cache with url1 as key should be retain"
                )
                assertNull(
                    storage.find(url2, emptyMap()),
                    "The cache with url2 as key should be removed"
                )
                assertNotNull(
                    storage.find(url3, emptyMap()),
                    "The cache with url3 as key should be retain"
                )
            }
        } finally {
            runBlocking {
